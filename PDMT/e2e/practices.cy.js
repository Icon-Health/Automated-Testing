describe('Provider testcases', () => {
    ...
    const prividerUpdateUrl = "https://pdmt-fh6wy6rlva-uc.a.run.app/practice/1811926983/update"

    beforeEach(() => {
        const url = "https://pdmt-fh6wy6rlva-uc.a.run.app/practice"
        cy.visit(url)
    })

    it('Practice Full list', () => {
        const expectedResults = 15374
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    })

    it("Practice Geo search", () => {
    ...
    it("Geo search with expanded radius ", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("select[name=dist]").first().select("15", { force: true }).should('have.value', '15')
        form.submit();

        const expectedResults = 96
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    })
    ...
    it("Geo search with expanded radius + name", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("input[name=name]").first().clear().type("Sports").should('have.value', 'Sports')
        cy.get("select[name=dist]").first().select("15", { force: true }).should('have.value', '15')
        form.submit();
    ...
    it("NPI - search", () => {
        cy.wait(3000)
        const form = cy.get("#form1")
        cy.get("input[name=name]").first().clear().type("1811926983").should('have.value', '1811926983')
        form.submit();
        cy.url().should('include', '/practice/1811926983')
    })

    it("Update Practice :  enter website, contact info, notes, name", () => {
