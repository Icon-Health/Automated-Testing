describe('Provider testcases', () => {
    ...
    const prividerUpdateUrl = "https://pdmt-fh6wy6rlva-uc.a.run.app/urgentcare/1316303340/update"

    beforeEach(() => {
        const url = "https://pdmt-fh6wy6rlva-uc.a.run.app/urgentcare"
        cy.visit(url)
    })

    it('UC Full list', () => {
        const expectedResults = 9821
    ...
    it("Geo search with expanded radius ", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("select[name=dist]").first().select("25", { force: true }).should('have.value', '25')
        form.submit();

        const expectedResults = 163
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    })
    ...
    it("Geo search with expanded radius + name", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("input[name=name]").first().clear().type("City").should('have.value', 'City')
        cy.get("select[name=dist]").first().select("20", { force: true }).should('have.value', '20')
        form.submit();
    ...
    it("NPI - search", () => {
        cy.wait(3000)
        const form = cy.get("#form1")
        cy.get("input[name=name]").first().clear().type("1316303340").should('have.value', '1316303340')
        form.submit();
        cy.url().should('include', '/urgentcare/1316303340')
    })

    it("Update UC :  enter website, contact info, notes, name", () => {
