describe('Provider testcases', () => {
    ...
    const prividerUpdateUrl = "https://pdmt-fh6wy6rlva-uc.a.run.app/provider/1518184605/update"

    beforeEach(() => {
        const url = "https://pdmt-fh6wy6rlva-uc.a.run.app"
        cy.visit(url)
    })

    it('Physician Full list', () => {
        const expectedResults = 61579
    ...
    it("Combo geo and name", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("input[name=name]").first().clear().type("Mark").should('have.value', 'Mark')
            // cy.wait(2000)
        form.submit();
    ...
    it("Combo geo and name and quality", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("input[name=name]").first().clear().type("Mark").should('have.value', 'Mark')
        cy.get("select[name=qual]").first().select("4", { force: true }).should('have.value', '4')
        form.submit();
    ...
    it("NPI - search", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("input[name=name]").first().clear().type("1740640051").should('have.value', '1740640051')
        form.submit();
        cy.url().should('include', '/provider/1740640051')
    })

    it("Specialty Search", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("#spec").select("Sports Knee").should('have.value', 'Sports Knee')
        form.submit();
        const expectedResults = 3103
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    })

    it("Specialty Search + Geo search", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("10101").should('have.value', '10101')
        cy.get("#spec").select("Sports Knee").should('have.value', 'Sports Knee')
        form.submit();
    ...
    it("Specialty Search + Geo search", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("10467").should('have.value', '10467')
        cy.get("#spec").select("Sports Knee").should('have.value', 'Sports Knee')
        form.submit();
        const expectedResults = 4
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    ...
    it("Specialty Search + Geo search + Increased Radius", () => {
        cy.wait(300)
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("10467").should('have.value', '10467')
        cy.get("#spec").select("Sports Knee").should('have.value', 'Sports Knee')
        cy.get("#dist").select("15").should('have.value', '15')
        form.submit();
        const expectedResults = 80
        cy.contains("Total Records").should('have.text', 'Total Records: ' + expectedResults)
    })

    it("Update physician: mark all options as No, enter website, contact info, notes", () => {
        cy.visit(prividerUpdateUrl)
        cy.get(".btn-group > input[value=no]").click({ multiple: true, force: true })
