describe('Provider testcases', () => {
    ...
    const prividerUpdateUrl = "https://pdmt-fh6wy6rlva-uc.a.run.app/imaging/1417255381/update"

    beforeEach(() => {
        const url = "https://pdmt-fh6wy6rlva-uc.a.run.app/imaging"
        cy.visit(url)
    })

    it('Img Full list', () => {
        const expectedResults = 6737
    ...
    it("Geo search with expanded radius + name", () => {
        ...
        const form = cy.get("#form1")
        cy.get("input[name=address]").first().clear().type("Greenwich CT").should('have.value', 'Greenwich CT')
        cy.get("input[name=name]").first().clear().type("New York").should('have.value', 'New York')
        cy.get("select[name=dist]").first().select("20", { force: true }).should('have.value', '20')
        form.submit();
    ...
    it("NPI - search", () => {
        cy.wait(3000)
        const form = cy.get("#form1")
        cy.get("input[name=name]").first().clear().type("1417255381").should('have.value', '1417255381')
        form.submit();
        cy.url().should('include', '/imaging/1417255381')
    })

    it("Update Img :  enter website, contact info, notes, name", () => {
